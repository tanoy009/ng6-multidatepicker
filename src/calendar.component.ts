...
export class CalendarComponent implements OnInit,OnChanges {
  ...
  private setDayObject(dayNumber: number, month: number, year: number): object {
  ...
  private daysInBetween = function(date1_ms, date2_ms): number {
  ...
  private setExternalDataWithEachDayObj(dayObj): IdateObject {
    ...
    if (!this.enableRangeSelect) {
      ...
      if(this.fromTimeClicked) {
        ...
        if(this.rangeSelected.to.day) {
          let _toDatObj = Object.assign({}, this.rangeSelected.to.day);
          let _tempPastDate = this.isPastDate(dayObject.day, dayObject.month, dayObject.year, _toDatObj.year, _toDatObj.month, _toDatObj.day);
          let _insideLimit = !this.maximumDayInRange || (this.daysInBetween(this.rangeSelected.from.day.dateInMillisecond , _toDatObj.dateInMillisecond) <= this.maximumDayInRange);
